...
- QA-002 dashboard tour auto-open on every login (trust + annoyance risk)
...
- ESLint warnings (non-fatal) remain across the repo; do not block deploy but should be cleaned post-launch.
- Repo-wide `tsc` may fail due to unrelated `.next/types` validator artifacts; Next build is green, so deploy readiness is maintained.
...
3. **State update patterns triggering strict lint rules** (`setState` inside effect)
