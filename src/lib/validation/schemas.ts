...
export const personaSchema = z.object({
  ...
  behaviors: z.array(z.string().min(18)).min(3).max(7),
  ...
  /**
   * Two concrete past episodes (fixed length for LLM JSON schema compatibility — not z.tuple).
   */
  formativeExperiences: z
    .array(z.string().min(20).max(450))
    .min(2)
    .max(2),
  /** Something they do repeatedly (ritual, tic, routine). */
  recurringHabit: z.string().min(15).max(320),
  /** 1–2 subtle inner contradictions (both should feel true). */
  contradictions: z.array(z.string().min(18).max(300)).min(1).max(2),
  habits: z.array(z.string().min(12).max(240)).min(2).max(4),
  /** Strong stances — specific, debatable, not platitudes. */
  opinions: z.array(z.string().min(16).max(360)).min(1).max(3),
  quirks: z.array(z.string().min(12).max(220)).min(2).max(5),
  /** How they actually write/speak: rhythm, punctuation, emoji policy, sentence length mix. */
  communicationFingerprint: z.string().min(35).max(650),
  /** Mild bias, irrational preference, or inconsistency humans have. */
  cognitiveBiasOrIrrationalStreak: z.string().min(18).max(360),
